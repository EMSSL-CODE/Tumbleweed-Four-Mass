...
// Enter Test case number (1, 2 or 3) in the #define test_case from choices below
// test_case : 1  ==    Max speed test
// test_case : 2  ==    kP Test
// test_case : 3  ==    Controlled Speed Test         SP_angvel  = 30 / 50 / 70 / 90
...
//============================================================================================================================
#define kP_vel                   K_const/SP_angvel
...
//  // Constant Amplitude Acceleration
   ...
   if (test_case == 1)
   {
   continuous_setpoint_from_angle_max_speed_test(pid1,pid2,pid3,pid4,angular_data);
   }
   
   if (test_case == 2)
   {
   continuous_setpoint_from_angle_kP_test(pid1,pid2,pid3,pid4,angular_data);
   }
   
   if (test_case == 3)
   {
   continuous_setpoint_from_angle_controlled_speed_test(pid1,pid2,pid3,pid4,angular_data);
   }
 //=========================================================================//
...
void continuous_setpoint_from_angle_max_speed_test(pidInfo &pid1, pidInfo &pid2, pidInfo &pid3, pidInfo &pid4, angleData &data)
{
  
	if (abs(data.velocity)<=SP_angvel+100)   // Ridiculously high value
	{

		pid1.sp = min(SP_mean + 15*cos(data.curr_angle*PI/180), MAX_SETPOINT);
        pid1.sp = max(pid1.sp,MIN_SETPOINT);
        pid2.sp = min(SP_mean - 15*cos(data.curr_angle*PI/180), MAX_SETPOINT);
        pid2.sp = max(pid2.sp,MIN_SETPOINT);
        pid3.sp = min(SP_mean + 15*sin(data.curr_angle*PI/180), MAX_SETPOINT);
        pid3.sp = max(pid3.sp,MIN_SETPOINT);
        pid4.sp = min(SP_mean - 15*sin(data.curr_angle*PI/180), MAX_SETPOINT);
        pid4.sp = max(pid4.sp,MIN_SETPOINT);
    }
...
void continuous_setpoint_from_angle_kP_test(pidInfo &pid1, pidInfo &pid2, pidInfo &pid3, pidInfo &pid4, angleData &data)
{

  data.velocity_error = (SP_angvel - abs(data.velocity));
  

     if (abs(data.velocity) >= 2.5)   // near zero ang vel (Need to callibrate)
     {
...
void continuous_setpoint_from_angle_controlled_speed_test(pidInfo &pid1, pidInfo &pid2, pidInfo &pid3, pidInfo &pid4, angleData &data)
{
